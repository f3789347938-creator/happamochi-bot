...
- **Goal**: LINEグループ公式Botのゼロからの再構築版。既存の本番D1データベース(33万件超のメッセージ、7000件超のメンバー)に接続しつつ、危険人物リスト・旧ガチャ機能を除いた全機能を実装。もちの着せ替えガチャは旧ガチャとは別機能として追加。
...
## 実装済み機能
- **もちの着せ替えガチャ（衣装120種類・背景30種類）**:
  - **ランキングアイコン**はLINEプロフィール画像が初期設定。
    `設定` → `ランキングアイコン`（`ランキングアイコン` / `アイコン設定`でも可）から、
    ガチャで獲得した衣装を選択でき、LINE画像にも無料で戻せる。
    着せ替えの衣装とは別に保存され、ガチャ獲得だけではアイコンは変更されない。
    LINEの個人・パズル・サバイバルのランキングカードと各「もっと見る」ページに反映する。
    `migrations/0025_ranking_icons.sql`は設定用テーブルの追加のみ。既存利用者にもLINE画像が適用される。
  - `着せ替え`で現在の装備・所持一覧、`着せ替えガチャ`（`きせかえガチャ` / `ガチャ`も可）で確認画面。
    1回 **3,000ポイント**。Yesを押すまで消費せず、Noは確認を無効化する。
    確認は本人専用・10分間・1回限り。再送や連打で同じ確認が二重課金されない。
  - 衣装と背景を合わせた**未所持の150点から均等抽選・重複なし**。全種所持後は消費しない。
    初期もち `C000` と初期背景 `BG000` は別枠で無料。獲得しただけでは装備を切り替えない。
  - `衣装一覧` / `背景一覧`で全種類を試着可能。未所持品は装備不可。
    `衣装装備 C001` / `背景装備 BG001` のような番号指定、正確な名前指定にも対応。
    所持品の付け替え・試着は無料。衣装と背景は別々に装備する。
  - 衣装と背景はLINEのステータスと公開ステータスWebページに反映。ランキングは専用アイコン設定を使う。
    **順位・EXP・レベル計算は従来の累計EXPのまま**。週間ランキングへの変更なし。
    パズル・サバイバルのゲーム内衣装／スコアランキングとは別管理。
    送信済みカードは書き換えられないため、装備後はコマンドで新しいカードを表示する。
  - 初期のLINEステータスは `mega` 幅300px・高さ278px。青い順位帯、称号、丸いLINEプロフィール画像、
    レベル・EXPバー、運勢・保有ポイント、「着せ替え」「称号変更」の2つのボタンを表示する。
    衣装・背景を両方とも初期設定に戻すと、この表示に戻る。公開ステータスの初期表示もLINE画像を使う。
    衣装または背景を選んだ場合は `kilo` 幅260px・高さ332pxの着せ替えステータスになる。
    この場合は2:1画像、名前と称号を横並びにし、称号をタップすると変更できる。
    ランキングは `kilo` 幅260px・高さ326px。
    ランキングは青帯・薄水色の本文・白い順位カード3行・自分の順位・「ランキングをもっと見る」ボタン。
    各行に四角いアイコン、名前、レベルと累計EXPを表示。累計EXPと同順位の計算は維持。
    ガチャ・カードテーマは「着せ替え」から開ける。
  - 着せ替えホームは幅260px・高さ326px、ランキングアイコン設定は幅260px・高さ266px。
    現在の画像と設定を横並びにし、短い説明と高さ32pxのボタンで操作できる。
  ...
  - `migrations/0024_dressup.sql` は新規テーブル・索引・トリガーの追加のみ。
    1つのガチャ確定UPDATEとトリガーで、ポイント減算・アイテム付与・台帳・確認の消費を原子的に実行。
    既存DBのリセットや既存テーブルの変更は不要。
  ...
  - 検証: `node --test tests/dressup/*.test.mjs tests/game/*.test.mjs` と `npm run build`。
    ...
    `--screenshot` は追加でPlaywrightとChromiumが必要。HTMLプレビューはLINE実機表示の近似。
    本番公開には既存DBを維持したまま0024・0025の追加マイグレーションとアプリのデプロイが必要。
...
- **個人ステータス(レベル/EXP/ポイント)**: 加算対象のメッセージ1通につき **1 EXP**。
  基本ポイントは本文の有効文字数に応じて変わる。空白・URL・絵文字・記号を除き、文字や語句の繰り返しをまとめて判定する。
  グループと個別トークで残高・EXP・5秒の加算間隔を共有し、
  スタンプ・画像など文字以外のメッセージは基本1P。参加登録は不要。
  ...
  同じグループ内の他の人のメッセージに引用返信すると **＋2P**、合計は **1通最大10P**。
  元メッセージと投稿者を確認できた返信だけが対象で、自分への返信・投稿者不明・別グループの引用には付かない。
  前回加算から5秒未満の連投、または前回加算された本文と同じ・よく似た内容は、EXPもポイントも加算しない。
  空白や繰り返しで文字数を水増ししてもポイントの段階は上がらない。
  同じイベントの再受信は`exp_events`の一意キーで二重加算を防ぐ。日・時間単位の獲得上限は設けない。
...
- **もち軍団サバイバル(LINEの中で遊べるゲーム その2)**: `サバイバル` と送ると
  開ける、倒れるまでスコアに挑戦するゲーム。指でスライドして動くだけで
  攻撃は自動。引き継ぎパック v14 を移植したもの。
  - 実体は `public/static/survivor/`。ランキングは `/ranking/survivor`。
  - **認証**: 元パックは `oai-authenticated-user-id` ヘッダーをプレイヤーID
    として信用していたが、LINEでは自分で付けるだけで他人になりすませる。
    そのため、もち合体パズルと同じ `verifyLiffToken()`(アクセストークンを
    LINEに検証させる方式)に置き換えてある。**元の方式には絶対に戻さない。**
  - **戦績の検算**: `src/features/survivor.ts` の `validateReport()` が、
    生存時間から「湧く敵の上限」「スコアの上限」などを計算して照合する。
    ありえない値は拒否する。ただし上限の範囲内での改ざんは防げない
    (盤面を再計算しない限り不可能)。
  - **画像**: 元は PNG 6枚で 13.5MB あった。未使用だった `arena.png`(2.7MB)を
    削除し、残りは表示サイズに合わせて縮小＋可逆WebP化して **5.0MB** にした。
    ただし `characters-source` だけは `art.js` の `SPRITE_CROPS` が
    ピクセル座標を直書きしているため**原寸(1536x1024)のまま**。ここを縮小すると
    キャラの切り出し位置がズレる。
  - **LIFF**: `src/features/menu/gameLink.ts` の `SURVIVOR_LIFF_ID`。
    1つのLINEログインチャネルにLIFFアプリは30個まで追加できるので、
    もち合体パズルと同じチャネルに2個目を足すだけでよい
    (ミニアプリ化や審査は不要)。エンドポイントURLには
    `https://line-group-bbs.pages.dev/static/survivor/` を登録する。
    **空のあいだは通常のHTTPS URLで開き、ゲームは遊べるが記録は無効。**
  - DBは `survivor_players` / `survivor_runs`(`migrations/0019_survivor.sql`)。
    既存テーブルには一切触れていない。
...
## 実装していない機能(意図的に除外)
- 危険人物リスト機能
- 旧ガチャ機能(user_gacha_count / gacha_history)。新しいもちの着せ替えガチャとは別機能。
...
着せ替え                         - 衣装・背景の所持一覧と装備
着せ替えガチャ / ガチャ          - 3,000Pで1回(Yes/No確認・重複なし)
衣装一覧 / 背景一覧              - 図鑑・無料試着
衣装装備 C001 / 背景装備 BG001   - 所持品を装備(名前指定も可)
カードテーマ                     - 従来のカード配色の選択・購入
...
- **除外テーブル(意図的・危険人物リスト/旧ガチャ機能ごと除外)**: danger_list, user_gacha_count, gacha_history
...
- **Status**: ✅ 本番デプロイ済み(2026-09-08)。オセロの対局終了カードに
  「📊 オセロ戦績」ボタンを追加。
  - **狙い**: `オセロ戦績` というコマンドの存在を知らない人が多いので、
    対局が終わった直後に目に入る場所へ置いて気づいてもらう。
  - ボタンは **`message` アクション**。押すと本当に「オセロ戦績」が発言として
    送信されるので、**トークに残り、周りの人にもコマンド名が伝わる**。
    既存の `オセロ戦績` コマンドがそのまま動くので、専用の分岐は足していない。
    普通の発言と同じ扱いになるため EXP・ポイントも加算されるが、
    これはユーザーの明示的な判断で許容している。
  - 出るのは**押した本人の戦績**。対局者以外(観戦者)が押しても本人の戦績が出る。
  - ボタンは **終局(finished)のときだけ**出す。対局中・相手待ちでは出さない。
  - 未対局の人が押したときは「まだ対局結果がありません」だけで終わらせず、
    `オセロ開始` で遊べることも案内する(ボタン経由は初見の人が多いため)。
  - **⚠️ この作業中に既存の不具合を1件見つけて直した**: この盤面カードは
    8x8=64マスを個別の box で描くため**盤面だけで約27,000バイト**あり、
    Flexの上限30,000バイトぎりぎりで動いていた。表示名は20文字までだが
    結合絵文字(「👨‍👩‍👧‍👦」など)は1文字25バイト前後になるため、
    **両者がそういう名前だと実測30,557バイトで上限を超え、LINEが400を返して
    カードが1枚も届かない**状態だった(ボタン追加前から潜在していた)。
    `shortenName()` で名前を48バイトに収め(コードポイント単位で切るので
    絵文字は壊れない)、さらに万一膨らんだ場合は説明文→footer の順に
    落としてカード自体は必ず届くようにした。
  - 検証: `node scripts/othello-test.mjs`(38項目) …
    終局時だけボタンが出る / `message` アクションである / 既存コマンドが動く /
    未対局でも落ちない / **既存の盤面タップ(postback)が壊れていない** /
    長い名前でも上限を超えずボタンが残る。
    `node scripts/othello-flex-validate.mjs`(9件) … LINE公式の検証APIで全件受理
    (最大29,655バイト)。
  - オセロ専用の自動テストはこれまで1件も無かったので、この機会に新規作成した。
  - マイグレーションの追加・変更なし。戦績の記録処理・`othello_records`・
    称号付与・盤面のルール判定には一切触っていない。
  - ⚠️ **実機のLINEアプリでの表示・タップは未確認**(検証用の端末がないため)。
- **過去の状態**: 本番デプロイ済み(2026-09-08)。もち合体パズルを新版に差し替え、

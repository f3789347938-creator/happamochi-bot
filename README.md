...
- **もちの着せ替えガチャ（衣装120種類・背景30種類）**:
  ...
  - LINEのステータス・個人ランキング、および公開ステータス・個人ランキングWebページに反映。
    **順位・EXP・レベル計算は従来の累計EXPのまま**。週間ランキングへの変更なし。
    パズル・サバイバルのゲーム内衣装／スコアランキングとは別管理。
    送信済みカードは書き換えられないため、装備後はコマンドで新しいカードを表示する。
  - LINEステータスと個人ランキングは `kilo` 幅260pxのコンパクト表示。高さはステータス332px、ランキング242px。
    ステータスは2:1画像、名前と称号を横並びにし、称号をタップすると変更できる。
    個人ランキングは上位3人、本人の行を青枠で強調。累計EXPと同順位の計算は維持。
  ...
  - 検証: `node --test tests/dressup/*.test.mjs tests/game/*.test.mjs` と `npm run build`。
    ...
    本番公開には既存DBを維持したまま0024の追加マイグレーションとアプリのデプロイが必要。

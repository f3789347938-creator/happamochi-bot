...
import {
  cancelGameRewardRun, finishGameRewardRun, startGameRewardRun, type GameRewardResult,
} from './gameRewards'
...
export async function startRun(
  ...
  // If the preceding score save succeeded but its reward did not, settle that
  // validated result before replacing the session. A remaining DB failure must
  // leave it pending instead of silently forfeiting the earned points.
  const unsettled = await env.DB.prepare(
    `SELECT r.run_id, s.score, s.finished_at FROM game_reward_runs r
       JOIN survivor_runs s ON s.id = r.run_id AND s.owner = r.user_id
      WHERE r.game = 'survivor' AND r.user_id = ? AND r.status = 'pending'
        AND s.finished_at IS NOT NULL AND s.report IS NOT NULL`
  ).bind(user.userId).first<{ run_id: string; score: number; finished_at: number }>()
  if (unsettled) {
    await finishGameRewardRun(env, user, 'survivor', unsettled.run_id, unsettled.score, unsettled.finished_at)
  }
  ...
  // Register both records in one transaction. A partial start cannot become a
  // legacy (non-rewarded) run merely because the response was lost.
  await startGameRewardRun(env, user, 'survivor', body.id, now, [
    env.DB.prepare(
      `INSERT INTO survivor_runs (id, owner, mode, week, ruleset, weapon, seed, started_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT(id) DO NOTHING`
    ).bind(body.id, user.userId, mode, week.key, RULESET, weapon, seed, now),
  ])

  // Concurrent retries of the same ID must receive the original seed and
  // loadout, not values generated by whichever request returned last.
  const created = await env.DB.prepare('SELECT owner, mode, seed, weapon, started_at FROM survivor_runs WHERE id = ?')
    .bind(body.id).first<{ owner: string; mode: string; seed: number; weapon: string; started_at: number }>()
  if (!created || created.owner !== user.userId) throw fail(409, '出撃を開始できませんでした。もう一度お試しください。')
  return { id: body.id, mode: created.mode, seed: created.seed, weapon: created.weapon, week: weekAt(created.started_at) }
}
...
export async function abandonRun(env: LineEnv, user: VerifiedUser, id: unknown): Promise<void> {
  ...
  await cancelGameRewardRun(env, user, 'survivor', id)
}
...
export async function finishRun(
  ...
  now: number
): Promise<{ saved: true; best: number; rank: number | null; already?: boolean; reward: GameRewardResult }> {
  ...
  if (run.finished_at !== null) {
    ...
    // A successful score save followed by a failed reward is retried using the
    // stored validated outcome, never the new request's score or later time.
    const reward = await finishGameRewardRun(env, user, 'survivor', run.id, run.score, run.finished_at, true)
    return { saved: true, best: p?.best_score ?? 0, rank: await rankOf(env, p?.best_score ?? 0, p?.plays ?? 0), already: true, reward }
  }
  ...
  // A concurrent finish may have won the conditional UPDATE. Its stored result
  // is authoritative for both the score and the JST reward day.
  const saved = await env.DB.prepare('SELECT score, finished_at FROM survivor_runs WHERE id = ? AND owner = ?')
    .bind(run.id, user.userId).first<{ score: number; finished_at: number }>()
  if (!saved) throw fail(404, 'この出撃の記録が見つかりません。')
  const reward = await finishGameRewardRun(env, user, 'survivor', run.id, saved.score, saved.finished_at, true)
  return { saved: true, best, rank: await rankOf(env, best, p?.plays ?? 1), reward }
}

...
After the live-event fix and continuous monitoring rollout, a new test call lasted 9,481 ms. The actual LINE history recorded its notification **4,278 ms after the call end**, with text `通話時間：9秒`, no phone emoji, and exactly one matching notification. D1 marked it sent 4,344 ms after the end (the extra 66 ms is acknowledgement/recording time). Earlier measured end-to-notification times were approximately 47–62 seconds. This is one measured post-fix sample, not a guaranteed latency bound.

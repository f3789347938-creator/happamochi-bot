...
Worker version `1166b954-a8e0-48df-a828-5558673e8494` に配備。自動テスト174件とWranglerのビルド検証が成功した。

- `既読開始`：利用者の投稿時刻 `1789842123282`、開始返信のLINE時刻 `1789842125559`。差は2,277ms。
- `既読確認`：投稿時刻 `1789842131623`、名前付き一覧の返信時刻 `1789842133628`。差は2,005ms。
- 返信は「既読が確認できた人（1人）」と、検証した利用者の表示名を含んでいた。利用者自身も「名前が表示された」と回答した。
- 管理APIの履歴で、開始と一覧の返信がそれぞれ1件であることを確認した。

終了・複数利用者・24時間期限・再配信・送信結果不明の扱いは自動テストで確認している。実機で複数人全員の既読や全端末の網羅性を確認したという意味ではない。

...
| 既読確認 | 最後にセットした位置から既読を確認できた、現在のグループメンバーの名前と最終既読確認時刻を表示する。誰でも同じ結果を参照できる。 |
...
- 名前の横の「最終既読確認」は、保存された最新の `payload.timestamp`（`event_at`）を日本時間（Asia/Tokyo）の月日・24時間表記で表示する。初回に読んだ瞬間やWorkerの受信時刻ではない。時刻が欠ける記録には時刻を補わない。
